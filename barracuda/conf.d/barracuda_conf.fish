...
# OS Info
set my_os "Android" "Darwin" "Windows" "Debian" "Fedora"
for x in $my_os
...
##I like lsd better.  Better icons and better output of owner/group
alias ls "lsd"
...
if not set -q needed_apps
    ...
    set apps "pv" "rsync" "micro" "bat" "exa" "fzf" "lsd"
    for x in $apps
	if [ ! -f "$PATH/$x" ]
		...
		case "Android"
		    	apt install $x --yes
		case "Fedora"
			sudo dnf install $x -i
		case "Debian"
			sudo apt install $x -y
		case "*"
		    	sudo pkg install $x --yes
	    end
	end
    end
